...
export function AgencyGpsPanel({
  ...
  return (
    <OutletSection title="Live GPS" hint={`${inRangeCount}/${rows.length} in geofence`}>
        ...
        <div className="border-t border-[var(--iz-line)] bg-[var(--iz-panel)] px-3 py-2.5">
          ...
          {outletPins.length === 1 && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="iz-tiny mt-2 inline-flex items-center gap-1 text-[#1a73e8]"
            >
              <ExternalLink className="h-3 w-3" />
              Open {outletPins[0]!.outlet} in Google Maps
            </a>
          )}
        </div>
      </IzCard>

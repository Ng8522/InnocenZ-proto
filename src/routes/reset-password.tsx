...
function ResetPassword() {
  ...
  const { channel, identifier, portal } = Route.useSearch();
  const signInSearch = { mode: "signin" as const, portal };
  ...
  const submit = (e: React.FormEvent) => {
    ...
    navigate({ to: "/signin", search: signInSearch });
  };

  return (
    <PhoneFrame overlay={<Toasts />}>
      <div className="iz-welcome flex flex-1 flex-col">
        <button type="button" onClick={() => navigate({ to: "/signin", search: signInSearch })} className="iz-chip w-fit">
